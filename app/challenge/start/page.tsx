...
export default function ChallengeStart() {
    ...
    const fetchQuestions = async () => {
        ...
        try {
            const response = await fetch(
                `/api/challenge/questions?challenge_id=${challengeId}`,
                {
                    method: 'GET',
                    headers: {
    ...
    const updateQuestionInDatabase = async (
        ...
        try {
                ...
                setQuestions((prevQuestions) =>
                    prevQuestions.map((q) =>
                        q.id === currentQuestion.id
                            ? { ...q, user_answer: selectedAnswer, status: 'completed' }
                            : q
                    )
                );
            }
            ...
            console.error(
                'An unknown error occurred while updating the question:',
                error
            );
    ...
    const createChallengeHistoryRecord = async (aiSuggestion: String) => {
        ...
        try {
            ...
            setMessage(
                'An unknown error occurred while creating the challenge history.'
            );
        }
    };
    ...
    const updateChallengeStatistics = async () => {
        ...
        try {
            ...
            setMessage(
                'An unknown error occurred while updating the challenge statistics.'
            );
        }
    };
    ...
    const currentQuestion = questions[currentQuestionIndex];
    const correctAnswer =
        currentQuestion[currentQuestion.answer as keyof Question];

    return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-primaryBackground text-textColor">
            <div className="w-full max-w-2xl p-6 bg-secondaryBackground rounded-lg shadow-lg pb-20 relative">
                ...
                {questions && questions.length > 0 && (
                    <div>
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-6">
                            {(['choice_a', 'choice_b', 'choice_c', 'choice_d'] as const).map(
                                (choice, index) => (
                                    <button
                                        key={index}
                                        onClick={() =>
                                            handleAnswerSelection(currentQuestion[choice])
                                        }
                                        className={`p-4 rounded transition ${selectedAnswer
                                                ...
                                                : 'bg-buttonBackground hover:bg-buttonHover'
                                            }`}
                        ...
                        {selectedAnswer && (
                            <button
                                ...
                                className="p-4 bg-buttonBackground rounded hover:bg-buttonHover transition absolute bottom-4 right-6"
                                disabled={isFinishing}
                            >
                                {isFinishing ? <LoadingDots /> : 'Continue'}
                            </button>
                        )}
                    </div>
                )}
