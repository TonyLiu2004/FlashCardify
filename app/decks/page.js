...
import { Dialog, DialogContent, DialogActions, DialogTitle, Grid, Container, Typography, Box, TextField, Button } from "@mui/material"
...
export default function Decks() {
    ...
    const handleEdit = async (id, name, description) => {
        try {
            const updatedDecks = decks.map(deck => deck.id === id ? { ...deck, name, description } : deck);
            ...
            const response = await fetch('/api/deck', {
                ...
                body: JSON.stringify({ deck: { id, name, user_id: user?.id, description: description } }),
            });
    ...
    const handleDelete = async () => {
        if (currentDeck) {
            try {
                const response = await fetch(`/api/deck?deck_id=${currentDeck}&user_id=${user?.id}`, {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
                    },
            ...
            try {
                ...
                if (deckToClone) {
                    const response = await fetch('/api/deck', {
                        ...
                        body: JSON.stringify({ deck: { name: `${deckToClone.name} (Copy)`, description: deckToClone.description, user_id: user?.id } }),
                    });
                    ...
                    const updatedDecksResponse = await fetch(`/api/deck?user_id=${user?.id}`);
    ...
    return (
        <div style={{backgroundColor:"#e5e7eb", height:"100vh", paddingTop:"20px"}}>
            <Container maxWidth="100vw" sx={{mt: 14}}>
                ...
                <Grid container spacing={2} sx={{ mt: 1 }}>
                    {decks && decks.length > 0 ? (
                        decks.filter(deck => deck.name.toLowerCase().includes(filter.toLowerCase()))
                        .map((deck) => (
            ...
            <Modal
                ...
                onConfirm={() => {
                    if (currentDeck) {
                        handlePublish(currentDeck, currentDeckVisibility === 'public' ? 'private' : 'public');
                    }
                }}
